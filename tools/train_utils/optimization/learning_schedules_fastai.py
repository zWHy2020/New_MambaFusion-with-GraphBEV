...
class LRSchedulerStep(object):
    ...
    def step(self, step, epoch=None):
        if len(self.use_diff_scale) > 0:
            for start, end, func in self.lr_phases:
                if step >= start:
                    self.optimizer.lr = [func((step - start) / (end - start)) * scale for scale in self.use_diff_scale]
    ...
    def __init__(self, fai_optimizer, total_step, lr_max, moms, div_factor,
        ...
        mom_phases = ((0, partial(annealing_cos, *self.moms)),
                      (self.pct_start, partial(annealing_cos,
                                               *self.moms[::-1])))
        self.use_diff_scale = use_diff_scale
        if len(self.use_diff_scale) > 0:
            assert len(fai_optimizer.opt.param_groups) / 2 == len(self.use_diff_scale)
            fai_optimizer.lr, fai_optimizer.mom = [low_lr * scale for scale in self.use_diff_scale], self.moms[0]
        else:
