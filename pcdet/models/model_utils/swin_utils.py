...
class AdaptivePadding(nn.Module):
    ...
    def get_pad_shape(self, input_shape):
        ...
        kernel_h, kernel_w = self.kernel_size # [8, 8]
        stride_h, stride_w = self.stride # [8, 8]
        output_h = math.ceil(input_h / stride_h) # 32
        output_w = math.ceil(input_w / stride_w) # 88
    ...
    def forward(self, x):
        pad_h, pad_w = self.get_pad_shape(x.size()[-2:]) # [12, 3, 256, 704] -> [256, 704]
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.adap_padding:
            x = self.adap_padding(x) # 其实不需要padding
        #? 为什么使用大小为8的卷积核？
        x = self.projection(x) # [12, 3, 256, 704] -> [12, 128, 32, 88]
        out_size = (x.shape[2], x.shape[3]) # [32, 88]
        x = x.flatten(2).transpose(1, 2) # [12, 128, 32, 88] -> [12, 32*88, 128]
