...
class MambaFusion(Detector3DTemplate):
    ...
    def get_training_loss(self,batch_dict):
        ...
        loss_graphbev_global_align = batch_dict.get('loss_graphbev_global_align', None)
        if loss_graphbev_global_align is not None:
            loss_trans = loss_trans + loss_graphbev_global_align
        tb_dict = {
            'loss_trans': loss_trans.item(),
            **tb_dict
        }
        if loss_graphbev_global_align is not None:
            tb_dict['loss_graphbev_global_align'] = loss_graphbev_global_align.item()
