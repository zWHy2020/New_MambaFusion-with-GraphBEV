...
# pytorch cross scan =============
class CrossScanNew(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: torch.Tensor):
        B, C, L = x.shape
        # ctx.shape = (B, C, L)
        xs = x.new_empty((B, 2, C, L)) # [1, 2, 128, num_of_features]
        xs[:, 0] = x
...
class CrossMergeNew(torch.autograd.Function):
    @staticmethod
    def forward(ctx, ys: torch.Tensor):
        # ctx.shape = (H, W)
        # ys = ys.view(B, K, D, -1)
