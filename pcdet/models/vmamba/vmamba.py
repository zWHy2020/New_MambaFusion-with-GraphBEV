...
try:
    ...
    # * 新加入的部分
    from .csms6s import CrossMergeNew, CrossScanNew
except:
    from csm_triton import CrossScanTriton, CrossMergeTriton, CrossScanTriton1b1, getCSM
    from csm_triton import CrossScanTritonF, CrossMergeTritonF, CrossScanTriton1b1F
    from csms6s import CrossScan, CrossMerge
...
class LayerNorm2d(nn.LayerNorm):
    def forward(self, x: torch.Tensor):
        x = x.permute(0, 2, 3, 1) # [2, 96, 128, 184] -> [2, 128, 184, 96]
        x = nn.functional.layer_norm(x, self.normalized_shape, self.weight, self.bias, self.eps)
        x = x.permute(0, 3, 1, 2) # [2, 128, 184, 96] -> [2, 96, 128, 184]
        return x
...
class SS2Dv2:
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        x_proj_weight = self.x_proj_weight # [4, 8, 96]
        x_proj_bias = getattr(self, "x_proj_bias", None) # None
        dt_projs_weight = self.dt_projs_weight # [4, 96, 6]
        dt_projs_bias = self.dt_projs_bias # [4, 96]
        A_logs = self.A_logs # [384, 1]
        Ds = self.Ds # [384]
        delta_softplus = True
        out_norm = getattr(self, "out_norm", None) # LayerNorm2d((96,), eps=1e-05, elementwise_affine=True)
        channel_first = self.channel_first # True
        to_fp32 = lambda *args: (_a.to(torch.float32) for _a in args)

        B, D, H, W = x.shape # [2, 96, 128, 184]
        D, N = A_logs.shape # 384, 1
        K, D, R = dt_projs_weight.shape # 4, 96, 6
        L = H * W # 23552 num_features
        ...
        if cascade2d: # False
            ...
            xs = CrossScan.apply(x) # [2, 4, 96, num_features]
            if no_einsum: # True
                # x_dbl = F.conv1d(xs.view(B, -1, L), x_proj_weight.view(-1, D, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K) # [2, 4, 96, num_features] -> [2, 32, num_features]
                # dts, Bs, Cs = torch.split(x_dbl.view(B, K, -1, L), [R, N, N], dim=2) # [2, 32(4*8), num_features] -> [2, 4, 6, num_features], [2, 4, 1, num_features], [2, 4, 1, num_features]
                dts_Bs_weights = x_proj_weight.view(-1, D, 1)[:-(K*N)].contiguous() # [32, 96, 1]
                Cs_weights = x_proj_weight.view(-1, D, 1)[-(K*N):].contiguous() # [6, 96, 1]
                x_dbl = F.conv1d(xs.view(B, -1, L), dts_Bs_weights, groups=K) # [2, 32(4*8), num_features] -> [2, 384(4*96), num_features]
                dts, Bs = torch.split(x_dbl.view(B, K, -1, L), [R, N], dim=2) # [2, 384(4*96), num_features] -> [2, 4, 96, num_features], [2, 4, 1, num_features]
                Cs = F.conv1d(xs.view(B, -1, L), Cs_weights, groups=K) # [2, 6(4*1), num_features] -> [2, 4, 1, num_features]
                dts = F.conv1d(dts.contiguous().view(B, -1, L), dt_projs_weight.view(K * D, -1, 1), groups=K) # [2, 32(4*8), num_features] -> [2, 384(4*96), num_features]
            else:
            ...
            xs = xs.view(B, -1, L) # [2, 384(4*96), num_features]
            dts = dts.contiguous().view(B, -1, L) # [2, 384(4*96), num_features]
            As = -torch.exp(A_logs.to(torch.float)) # (k * c, d_state) [384, 1]
            Bs = Bs.contiguous().view(B, K, N, L) # [2, 4, 1, num_features]
            Cs = Cs.contiguous().view(B, K, N, L) # [2, 4, 1, num_features]
            Ds = Ds.to(torch.float) # (K * c) [384]
            delta_bias = dt_projs_bias.view(-1).to(torch.float) # (K * c) [384]

            if force_fp32: # False
                xs, dts, Bs, Cs = to_fp32(xs, dts, Bs, Cs)

            ys: torch.Tensor = selective_scan( # 这个是mamba的主函数
                xs, dts, As, Bs, Cs, Ds, delta_bias, delta_softplus
            ).view(B, K, -1, H, W) # [2, 4, 96, H, W]
            
            y: torch.Tensor = CrossMerge.apply(ys) # [2, 96, H, W] 4个维度就简单相加？？
        ...
        y = y.view(B, -1, H, W) # [2, 96, H, W]
        if not channel_first: # False
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        x = self.in_proj(x) # [2, 96, 128, 184] -> [2, 96, 128, 184]
        ...
        if self.with_dconv: # True
            x = self.conv2d(x) # (b, d, h, w) [2, 96, 128, 184] -> [2, 96, 128, 184]
        x = self.act(x) # [2, 96, 128, 184] -> [2, 96, 128, 184]
...
class Cross2d_v2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        # forward_type debug =======================================
        FORWARD_TYPES = dict(
            v01=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScanTriton, CrossMerge=CrossMergeTriton),
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        out = self.dropout(self.out_proj(y))
        return out
class FusionPositionEncodingNet(nn.Module):
    def __init__(self, in_channels, fusion_channels, out_channels):
        super(FusionPositionEncodingNet, self).__init__()
        self.conv1 = nn.Conv2d(in_channels * 2, fusion_channels, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(fusion_channels)
        self.conv2 = nn.Conv2d(fusion_channels, out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x1, x2):
        ...
        pos_encoding = self.bn2(self.conv2(x))
        return pos_encoding
class SS2D_Cross:
    def __init_Cross__(
        self,
        ...
        **kwargs,    
        ...
        # forward_type debug =======================================
        FORWARD_TYPES = dict(
            cross=partial(self.forward_core_Cross, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScanTriton, CrossMerge=CrossMergeTriton),
            # ===============================
        )
        self.forward_core = FORWARD_TYPES.get(forward_type, None)
        k_group = 4
        ...
        self.in_proj2 = Linear(d_model, d_cross, bias=bias)
        ...
        if self.with_embed:
            self.embed = FusionPositionEncodingNet(in_channels=d_model, fusion_channels=d_cross*2, out_channels=d_cross)
        # x proj ============================
        self.x_proj = [
            nn.Linear(d_cross, (dt_rank + d_state * 2), bias=False)
            for _ in range(k_group)
        ]
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0)) # (K, N, inner)
        del self.x_proj
    ...
    def forward_core_Cross(
        ...
        **kwargs,
        ...
        if cascade2d: # False
            ...
            if self.with_embed:
                embedded = self.embed(x, x_cross)
                if self.use_newroute:
                    x = embedded + x
                    x_cross = embedded + x_cross
                else:
                    x_cross = embedded + x_cross
            xs = CrossScan.apply(x) # [2, 4, 96, num_features] [1, 4, 128, 32400]
            xs_cross = CrossScan.apply(x_cross) # [2, 4, 128, num_features] [1, 4, 128, 32400]
            
            D_cross = xs_cross.shape[2] # 128
            if self.use_newroute:
                # stacked_xs = torch.stack((xs, xs_cross), dim=-1)
                interleaved_xs = torch.stack((xs, xs_cross), dim=-1).reshape(*xs.shape[:-1], L * 2) # [1, 4, 128, 64800]
                # separated = interleaved_xs.view(B, K, D_cross,  -1)
                # separated_xs = separated[..., ::2].view(B, K, D_cross, H * W)
                # assert torch.equal(separated_xs, xs)
                xs = interleaved_xs
                # xs_recover = interleaved_xs
                L = L * 2
            if no_einsum: # True
                if self.use_newroute:
                    x_dbl = F.conv1d(xs.view(B, -1, L), x_proj_weight.view(-1, D, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K) # [2, 4, 96, num_features] -> [2, 32, num_features]
                else:
                    x_dbl = F.conv1d(xs_cross.view(B, -1, L), x_proj_weight.view(-1, D_cross, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K) # [2, 4, 96, num_features] -> [2, 32, num_features]
                dts, Bs, Cs = torch.split(x_dbl.view(B, K, -1, L), [R, N, N], dim=2) # [2, 32(4*8), num_features] -> [2, 4, 6, num_features], [2, 4, 1, num_features], [2, 4, 1, num_features]
                dts = F.conv1d(dts.contiguous().view(B, -1, L), dt_projs_weight.view(K * D, -1, 1), groups=K) # [2, 32(4*8), num_features] -> [2, 384(4*96), num_features]
            else:
                x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, x_proj_weight)
            ...
            if self.use_newroute:
                ...
                separated = ys.view(B, K, D_cross,  -1) # [2, 4, 96, H * W]
                ys = separated[..., ::2].view(B, K, D_cross, H, W)
                # ys_cross = separated[..., 1::2].view(B, K, D_cross, H, W)
            else:
                ys: torch.Tensor = selective_scan( # 这个是mamba的主函数
                    xs, dts, As, Bs, Cs, Ds, delta_bias, delta_softplus
                ).view(B, K, -1, H, W) # [2, 4, 96, H, W]
    ...
    def forward_Cross(self, x: torch.Tensor, x_cross: torch.Tensor, **kwargs):
        x = self.in_proj(x) # [2, 96, 128, 184] -> [2, 96, 128, 184]
        x_cross = self.in_proj2(x_cross) # [2, 96, 128, 184] -> [2, 96, 128, 184]
        if not self.disable_z: # False
            x, z = x.chunk(2, dim=(1 if self.channel_first else -1)) # (b, h, w, d)
            if not self.disable_z_act:
                z = self.act(z)
        if not self.channel_first: # False
            x = x.permute(0, 3, 1, 2).contiguous()
            x_cross = x_cross.permute(0, 3, 1, 2).contiguous()
        if self.with_dconv: # True
            x = self.conv2d(x) # (b, d, h, w) [2, 96, 128, 184] -> [2, 96, 128, 184]
            x_cross = self.conv2d2(x_cross) # (b, d, h, w) [2, 96, 128, 184] -> [2, 96, 128, 184]
        x = self.act(x) # [2, 96, 128, 184] -> [2, 96, 128, 184]
        x_cross = self.act2(x_cross) # [2, 96, 128, 184] -> [2, 96, 128, 184]
        y = self.forward_core(x, x_cross)
        y = self.out_act(y)
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y))
...
class SS1D:
    def __initv_1d__(
        ...
        **kwargs,    
        ...
        FORWARD_TYPES = dict(
            # v05=partial(self.forward_corev2, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScanTriton, CrossMerge=CrossMergeTriton),
            v1d=partial(self.forward_corev_1d, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScanNew, CrossMerge=CrossMergeNew),
            v1dcross=partial(self.forward_corev_1d, force_fp32=False, SelectiveScan=SelectiveScanOflex, no_einsum=True, CrossScan=CrossScanNew, CrossMerge=CrossMergeNew),
        )
        self.forward_core = FORWARD_TYPES.get(forward_type, None)
        k_group = 2
        ...
        if initialize in ["v0"]:
            ...
            self.dt_projs_weight = nn.Parameter(torch.stack([t.weight for t in self.dt_projs], dim=0)) # (K, inner, rank) [2, 128, 8]
            self.dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in self.dt_projs], dim=0)) # (K, inner) [2, 128]
            del self.dt_projs
    ...
    def forward_corev_1d(
        ...
        **kwargs,
        ...
        x_proj_weight = self.x_proj_weight # [4, 8, 96] [2, 10, 128]
        x_proj_bias = getattr(self, "x_proj_bias", None) # None
        dt_projs_weight = self.dt_projs_weight # [4, 96, 6] [2, 128, 8]
        dt_projs_bias = self.dt_projs_bias # [4, 96] [2, 128]
        A_logs = self.A_logs # [384, 1] [256, 1]
        Ds = self.Ds # [384] [256]
        delta_softplus = True
        out_norm = getattr(self, "out_norm", None) # LayerNorm2d((96,), eps=1e-05, elementwise_affine=True) LayerNorm((128,), eps=1e-05, elementwise_affine=True)
        channel_first = self.channel_first # True False
        to_fp32 = lambda *args: (_a.to(torch.float32) for _a in args)

        B, D, L = x.shape # [2, 96, 128, 184] (1, 15272, 256)
        D, N = A_logs.shape # 384, 1 256, 1
        K, D, R = dt_projs_weight.shape # 4, 96, 6 (2, 128, 8)
        # L = H * W # 23552 num_features
        ...
        if cascade2d: # False
            ...
            xs = CrossScan.apply(x) # [2, 4, 96, num_features] [1, 2, 128, 23606]
            if x_cross is not None:
                xs_cross = CrossScan.apply(x_cross)
                D_cross = xs_cross.shape[2]
            if no_einsum: # True
                # [1, 256, 23606] [20, 128, 1] 2 -> [1, 20, 23606]
                if x_cross is not None:
                    x_dbl = F.conv1d(xs_cross.view(B, -1, L), x_proj_weight.view(-1, D_cross, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K) # [2, 4, 96, num_features] -> [2, 32, num_features]
                else:
                    x_dbl = F.conv1d(xs.view(B, -1, L), x_proj_weight.view(-1, D, 1), bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K) # [2, 4, 96, num_features] -> [2, 32, num_features]
                # [1, 2, 10, 23606] [8, 1, 1] -> [1, 2, 8, 23606], [1, 2, 1, 23606], [1, 2, 1, 23606]
                dts, Bs, Cs = torch.split(x_dbl.view(B, K, -1, L), [R, N, N], dim=2) # [2, 32(4*8), num_features] -> [2, 4, 6, num_features], [2, 4, 1, num_features], [2, 4, 1, num_features]
                # [1, 256, 23606] [256, 8, 1] 2 -> [1, 256, 23606]
                dts = F.conv1d(dts.contiguous().view(B, -1, L), dt_projs_weight.view(K * D, -1, 1), groups=K) # [2, 32(4*8), num_features] -> [2, 384(4*96), num_features]
            else:
                x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, x_proj_weight)
                if x_proj_bias is not None:
        ...
        y = y.view(B, -1, L) # [2, 96, H, W] [1, 128, 7456]
        if not channel_first: # False
            y = y.view(B, -1, L).transpose(dim0=1, dim1=2).contiguous().view(B, L, -1) # (B, L, C) # [1, 7456, 128]
        y = out_norm(y)

        return (y.to(x.dtype) if to_dtype else y)

    def forward_1d(self, x: torch.Tensor, x_cross=None, **kwargs):
        x = self.in_proj(x) # [8546, 128] -> [8546, 128]
        if len(x.shape)==2:
            x = x.unsqueeze(0) # [8546, 128] -> [1, 8546, 128]
            x_cross = x_cross.unsqueeze(0) if x_cross is not None else None
        x = x.permute(0, 2, 1).contiguous() # [1, 8546, 128] -> [1, 128, 8546]
        x_cross = x_cross.permute(0, 2, 1).contiguous() if x_cross is not None else None
        # if not self.disable_z: # False
        #     x, z = x.chunk(2, dim=(1 if self.channel_first else -1)) # 
        #     if not self.disable_z_act:
        #         z = self.act(z)
        ...
        x = self.act(x) # SiLU()
        y = self.forward_core(x, x_cross)
        y = self.out_act(y) # Identity() [1, 20136, 128]
        # if not self.disable_z:
        #     y = y * z
        out = self.dropout(self.out_proj(y))
        return out
...
class SS2D(nn.Module, mamba_init, SS2Dv0, SS2Dv2, SS2Dv3, SS1D, SS2D_Cross, Cross2d_v2):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
        ...
        **kwargs,
        ...
        elif forward_type.startswith("v1d"):
            if forward_type.startswith("v1dcross"):
                kwargs.update(d_cross=d_cross)
            self.__initv_1d__(**kwargs)
        elif forward_type.startswith("cross"):
            kwargs.update(d_cross=d_cross)
            self.__init_Cross__(**kwargs)
        else:
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        if type(input) is tuple:
            ...
            x = input # [2, 96, 128, 184]
            cross_flag = False
        if self.ssm_branch:
            if self.post_norm:
                x = (x_cross if cross_flag and self.use_cross_input else x) + self.drop_path(self.norm(self.op(x)))
            else: # True
                x = (x_cross if cross_flag and self.use_cross_input else x) + \
                    (self.drop_path(self.op(self.norm(x), x_cross) if cross_flag else self.op(self.norm(x)))) # [2, 96, H, W]
        if self.mlp_branch:
            if self.post_norm:
                x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            else: # True
                x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN [2, 96, H, W]
        return x
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        def layer_forward(l, x):
            x = l.blocks(x) # [2, 96, H, W]
            y = l.downsample(x) # [2, 96*2, H/2, W/2]
            return x, y

        x = self.patch_embed(x) # [2, 3, 512, 736] -> [2, 96, 128, 184]
        outs = []
        for i, layer in enumerate(self.layers):
            o, x = layer_forward(layer, x) # (B, H, W, C) [2, 96, H, W] [2, 96*2, H/2, W/2]
    ...
    def step(self, x, i):
        layer = self.layers[i]
        if i != 0:
            layer_before = self.layers[i - 1]
            x = layer_before.downsample(x)
        x = layer.blocks(x)
