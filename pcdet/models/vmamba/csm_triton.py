...
def triton_cross_scan(
    ...
    i_hw, i_c, i_b = tl.program_id(0), tl.program_id(1), tl.program_id(2) # [12, 96, 2]
    i_h, i_w = (i_hw // NW), (i_hw % NW) # 通过高宽块索引计算得到的具体的高度块和宽度块索引。
    _mask_h = (i_h * BH + tl.arange(0, BH)) < DH
    _mask_w = (i_w * BW + tl.arange(0, BW)) < DW # 分别计算高度和宽度的掩码，用于确定当前块在实际数据维度内的有效部分。
    _mask_hw = _mask_h[:, None] & _mask_w[None, :] # 结合高度和宽度掩码得到一个二维掩码，用于处理边界内的数据。
    _for_C = min(DC - i_c * BC, BC)

    _tmp0 = i_c * BC * DH * DW
    _tmp1 = DC * DH * DW
    _tmp2 = _tmp0 + i_h * BH * DW  + tl.arange(0, BH)[:, None] * DW + i_w * BW + tl.arange(0, BW)[None, :] # 计算基于块大小和块索引的内存偏移量，用于定位数据。
    ...
    # 计算具体的数据读写位置，包括原始数据 (p_y1)，以及不同变换的版本 (p_y2, p_y3, p_y4)。
    for idxc in range(_for_C): # 循环遍历通道块中的每个通道，并对每个通道执行数据加载和存储。
        _idx = idxc * DH * DW
        _x = tl.load(p_x + _idx, mask=_mask_hw)
        tl.store(p_y1 + _idx, _x, mask=_mask_hw) # 原始数据
        tl.store(p_y2 + _idx, _x, mask=_mask_hw) # 水平翻转
        tl.store(p_y3 + _idx, _x, mask=_mask_hw) # 垂直翻转
        tl.store(p_y4 + _idx, _x, mask=_mask_hw) # 水平垂直翻转
...
class CrossScanTriton(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor):
        B, C, H, W = x.shape # [2, 96, 128, 184]
        B, C, H, W = int(B), int(C), int(H), int(W)
        BC, BH, BW = min(triton.next_power_of_2(C), 1), min(triton.next_power_of_2(H), 64), min(triton.next_power_of_2(W), 64) # 1, 64, 64
        NH, NW, NC = triton.cdiv(H, BH), triton.cdiv(W, BW), triton.cdiv(C, BC) # 3, 4, 96
        ctx.shape = (B, C, H, W)
        ctx.triton_shape = (BC, BH, BW, NC, NH, NW)
        x = x.contiguous() # [2, 96, 128, 184]
        y = x.new_empty((B, 4, C, H, W)) # [2, 4, 96, 128, 184]
        triton_cross_scan[(NH * NW, NC, B)](x, y, BC, BH, BW, C, H, W, NH, NW) # [12, 96, 2] (x, y, 1, 64, 64, 96, 128, 184, 3, 4)
        return y.view(B, 4, C, -1) # [2, 4, 96, 23552] 4种不同顺序的特征
...
class CrossScanTritonNewroute(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: torch.Tensor):
        B, C, H, W = x.shape # [2, 96, 128, 184]
        B, C, H, W = int(B), int(C), int(H), int(W)
