...
class DataProcessor(object):
    ...
    def image_normalize(self, data_dict=None, config=None):
        ...
        use_img_debug = config.get('use_debug_img', False)
        if use_img_debug:
            data_dict['auged_img'] = []
            for img in data_dict['camera_imgs']:
                data_dict['auged_img'].append(np.array(img))
        mean = config.mean
        std = config.std
        ...
        if 'img_sweeps' in data_dict:
            new_img_sweeps = []
            for imgs in data_dict["img_sweeps"]:
                new_img_sweeps.append([compose(img) for img in imgs])
            data_dict["img_sweeps"] = new_img_sweeps
        return data_dict
