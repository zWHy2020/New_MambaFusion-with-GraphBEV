...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10, with_cam=False):
    ...
    for index, sample in enumerate(nusc.sample):
        ...
        with_img_sweeps = True
        sample_data_token = sample['data'][chan]
        curr_sd_rec = nusc.get('sample_data', sample_data_token)
        sweeps = []
        if with_cam and with_img_sweeps:
            sample_data_token_img_list = []
            for cam in camera_types:
                sample_data_token_img = sample['data'][cam]
                sample_data_token_img_list.append(sample_data_token_img)
            ref_img_path_list = [nusc.get_sample_data_path(token) for token in sample_data_token_img_list]
            curr_sd_rec_img_list = [nusc.get('sample_data', token) for token in sample_data_token_img_list]
            sweeps_img = []
        ...
        while len(sweeps) < max_sweeps - 1:
            if curr_sd_rec['prev'] == '':
                if len(sweeps) == 0:
                    ...
                    if with_cam and with_img_sweeps:
                        sweep_img = {}
                        for cam, img_path, sd_rec_img in zip(camera_types, ref_img_path_list, curr_sd_rec_img_list):
                            sweep_img[cam] = {
                                'img_path': Path(img_path).relative_to(data_path).__str__(),
                                'sample_data_token': sd_rec_img['token'],
                                'transform_matrix': None,
                                'time_lag': sd_rec_img['timestamp'] * 0,
                            }
                        sweeps_img.append(sweep_img)
                else:
                    sweeps.append(sweeps[-1])
                    if with_cam and with_img_sweeps:
                        sweeps_img.append(sweeps_img[-1])
            else:
                ...
                if with_cam and with_img_sweeps:
                    ...
                    for cam, img_path, sd_rec_img in zip(camera_types, ref_img_path_list, curr_sd_rec_img_list):
                        cam_info = obtain_sensor2top(
                            nusc, sd_rec_img['token'], l2e_t, l2e_r_mat, e2g_t, e2g_r_mat, cam
                        )
                        cam_info['data_path'] = Path(cam_info['data_path']).relative_to(data_path).__str__()
                        tm_img = reduce(np.dot, [ref_from_car, car_from_global, global_from_car, cam_info['sensor2ego_rotation']])
                        time_lag_img = sd_rec_img['timestamp'] * 1e-6 - ref_time
                        sweep_img[cam] = {
                            'img_path': Path(img_path).relative_to(data_path).__str__(),
                            'sample_data_token': sd_rec_img['token'],
                            'transform_matrix': tm_img,
                            'time_lag': time_lag_img,
                        }
                    sweeps_img.append(sweep_img)
